...
(defun print-plan-node (node stream depth)
...
(defun print-causal-link (link stream depth)
  (declare (ignore depth))
  (princ "<" stream)
      ...
      (cond ((null undercutting-interest)
             (setf undercutting-interest
                   (make-interest
                     ...
                     :interest-defeater-binding (support-link-binding link)
                     :interest-generating-defeat-nodes (list (support-link-target link))
                     :interest-decision-plans
            ...
            (t 
              ...
              (push (support-link-target link) (interest-generating-defeat-nodes undercutting-interest))
              (when (eq (support-link-rule link) protoplan)
                (push (mem2 (node-formula (support-link-target link)))
                      (interest-decision-plans undercutting-interest)))
              (dolist (in (hypernode-discharged-interests (support-link-target link)))
                (dolist (p (interest-decision-plans (car in)))
                  (pushnew p (interest-decision-plans undercutting-interest))))
    ...
    (multiple-value-bind
      ...
      (cond ((null rebutting-interest)
            ...
            (t 
              ...
              (push (support-link-target link) (interest-generating-defeat-nodes rebutting-interest))
              (when (eq (support-link-rule link) protoplan)
                (push (mem2 (node-formula (support-link-target link)))
                      (interest-decision-plans rebutting-interest)))
              (dolist (in (hypernode-discharged-interests (support-link-target link)))
                (dolist (p (interest-decision-plans (car in)))
                  (pushnew p (interest-decision-plans rebutting-interest))))
                ...
                (every
                  #'(lambda (Q)
                      (or (not (eq (item-kind Q) :interest))
                          (not (member p (interest-decision-plans (enqued-item Q))))))
                  *inference-queue*)))
          *plans-decided*)
    (throw 'die nil))
