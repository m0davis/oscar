...
data Problem 
    =   Problem (Id (Plural Condition) (Plural Satisfaction) 
    |   (Maybe Attribution))
    |   ProblemDescription
...
sentences are modeled by conditions
given the conditions, satisfy
...
class (Coercible a b, Coercible b c, Coercible c b, Coercible b a) => BiCoercible a b c | a c -> b where
    forwards :: a -> c
    backwards :: c -> a
...
instance BiCoercible A Text B where
    forwards = (coerce :: Text -> B) . (coerce :: A -> Text)
    backwards = (coerce :: Text -> A) . (coerce :: B -> Text)

foo :: (BiCoercible a Text b, a ~ A, b ~ B) => b -> a
foo = backwards

doit :: IO ()
doit = print $ foo (B . pack $ "bar")
...
type family ProblemReasonsText (direction :: Direction) (defeasible :: Defeasibility) :: *

type instance ProblemReasonsText Forwards  PrimaFacie = ProblemSectionText ForwardsPrimaFacieReasons
type instance ProblemReasonsText Backwards PrimaFacie = ProblemSectionText BackwardsPrimaFacieReasons
type instance ProblemReasonsText Forwards  Conclusive = ProblemSectionText ForwardsConclusiveReasons
type instance ProblemReasonsText Backwards Conclusive = ProblemSectionText BackwardsConclusiveReasons

--data family ProblemReasonsText (direction :: Direction) (defeasible :: Defeasibility) :: *

--data instance ProblemReasonsText Forwards PrimaFacie = ProblemForwardsPrimaFacieReasonsText
--data instance ProblemReasonsText Backwards PrimaFacie = ProblemBackwardsPrimaFacieReasonsText
--data instance ProblemReasonsText Forwards Conclusive = ProblemForwardsConclusiveReasonsText
--data instance ProblemReasonsText Backwards Conclusive = ProblemBackwardsConclusiveReasonsText




fromReasonBlocks' :: forall direction defeasible prt out. 
    (   Coercible (ProblemReasonsText direction defeasible) Text
    ,   Coercible Text (ProblemReasonText direction defeasible)
    ,   prt ~ ProblemReasonsText direction defeasible
    ,   out ~ ProblemReasonText direction defeasible
    )   => 
    prt -> 
    [(ProblemReasonName, out, ProblemVariablesText, ProblemStrengthDegree)]
fromReasonBlocks' = either (error . ppShow) id . runParser (many (try p) <* many space <* eof) () "" . coerce
    where
        --p :: Parser (ProblemReasonName, ProblemReasonText direction defeasible, ProblemVariablesText, ProblemStrengthDegree)
        p = do
...
given
    a character, c, that is from the set CHAR - ' '
...
Maybe is like a functin from a set RedThings -> SetOfAppleNames Union emptyset s.t. 



'produce a' is immediately followed by 'produce b'

parameters: number of times to write ' ', n
...
given
    a problem number
    a description :: no space at head or last
...
--        parseFormula
--            `priorTo` 
--                (many space >> string "justification" >> many space >> char '=' >> many space >> (double `priorTo` (many space >> newline)))
...
import Text.ParserCombinators.Poly.Text
...
skipSpaceChars :: Parser ()
skipSpaceChars = manySatisfy (== ' ') *> pure ()

newline :: Parser ()
newline = newlineLF {-<|> newlineCRLF-}

ae x = flip adjustErr ((x++": ")++)

newlineLF :: Parser ()
newlineLF = ae "newlineLF" $ satisfy (== '\n') *> pure ()

newlineCRLF :: Parser ()
newlineCRLF = ae "newlineCRLF" $ satisfy (== '\r') *> satisfy (== '\n') *> pure ()

matchString :: String -> Parser ()
matchString s = guardM $ return . (== s) =<< upto (length s) next
...
-- ends after the newline
problemNumberLine :: Parser ProblemNumber
problemNumberLine = matchString "Problem #" *> (ProblemNumber . read . (\x -> [x]) <$> {-many-} (satisfy isDigit)) <* emptyLine
...
problemDescription = 
    ProblemDescription . pack <$> 
        manyFinally' next
            (many emptyLine >> headerLine "Given premises:")

prFormula :: Parser Formula
prFormula = ae "formula" $ Formula . pack <$> many next

prDegree :: Parser Degree
prDegree = ae "degree" $ Degree . read <$> manyFinally' next emptyLine
...
premiseParser = ae "premiseParser" $do
    unlessM (isAhead $ matchString "Ultimate epistemic interests:" >> emptyLine) $ do
        (f, (_, d)) <- 
            between_ng
                skipSpaceChars 
                (prFormula) 
                (skipSpaceChars *> matchString "justification" *> skipSpaceChars *> matchString "=" *> skipSpaceChars *> 
                    prDegree 
                    <* (ae "?" $ emptyLine))
        return $ Premise f d

isAhead :: Parser a -> Parser Bool
isAhead p = (p *> pure True) `onFail` pure False

between_ng :: Parser open -> Parser between -> Parser close -> Parser (between, (open, close))
between_ng o b c = traceShow "between_ng!" $ do
    o' <- ae "btween" $ o
    interior <- ae "btween2" $ manyFinally' next (isAhead c)
    let (Right b', _) = traceShow interior $ runParser b (pack interior)
    c' <- c
    return (b', (o', c'))

problemParser :: Parser Problem
problemParser = traceShow "asdfasdf" $ do
    commit $ many emptyLine
    pn <- problemNumberLine
    return $ traceShow pn
    commit $ many emptyLine
    desc <- problemDescription
    traceShow desc $ do
        commit $ many emptyLine
        (prems, _) <- ae "pp" $ between_ng (ae "ppp" $ headerLine "Given premises:") (Set.fromList <$> many premiseParser) (ae "uei" $ headerLine "Ultimate epistemic interests:")
        return $ Problem pn desc prems


headerLine :: String -> Parser ()
headerLine s = do
    ae "hl1" $ skipSpaceChars
    ae ("hl2=" ++ s) $ matchString s
    ae ("hl3=" ++ s) $ skipSpaceChars
    ae ("hl4=" ++ s) $ newline

main :: IO ()
main = combinedProblemsFileReader >>= return . (\(x, y) -> (x, take 100 y)) . runParser problemParser >>= print
