...
Extended : ∀ {𝔞} {𝔄 : Ø 𝔞} {𝔟} {𝔅 : Ø 𝔟} {ℓ} → (π̂ ℓ 𝔅 ← 𝔅) → (𝔅 ← 𝔄) → (𝔄 → 𝔅) → Ø 𝔞 ∙̂ ℓ
Extended _≈_ = λ f g → ∀ x → f x ≈ g x

Ṗroperty : ∀ {𝔵} {𝔛 : Ø 𝔵} {𝔬} ℓ → (𝔵 ∙̂ 𝔬 ∙̂ ↑̂ ℓ) ←̂ (𝔬 ←̂ 𝔛)
Ṗroperty = ∀̇ π̂

LeftṖroperty : ∀ {𝔵} {𝔛 : Ø 𝔵} {𝔬₁} → (𝔛 → 𝔛 → Ø 𝔬₁) → ∀ ℓ → 𝔛 → Ø 𝔵 ∙̂ 𝔬₁ ∙̂ ↑̂ ℓ
LeftṖroperty ArrowO1O2 ℓ = ∀̇ π̂ ℓ ∘ ArrowO1O2

ArrowsourceṖroperty : ∀ {𝔵} {𝔛 : Ø 𝔵} {𝔬₁} {𝔬₂} → (𝔛 → Ø 𝔬₁) → (𝔛 → Ø 𝔬₂) → ∀ ℓ → 𝔛 → Ø 𝔵 ∙̂ 𝔬₁ ∙̂ 𝔬₂ ∙̂ ↑̂ ℓ
ArrowsourceṖroperty O1 O2 ℓ = LeftṖroperty (Arrow O1 O2) ℓ
...
ExtendedṖroperty : ∀ {𝔵} {𝔛 : Ø 𝔵} {𝔬} {ℓ̇}
  (𝔒 : 𝔛 → Ø 𝔬) ℓ (_↦_ : ∀̇ π̂² ℓ̇ 𝔒)
  → Ø 𝔵 ∙̂ 𝔬 ∙̂ ↑̂ ℓ ∙̂ ℓ̇
ExtendedṖroperty 𝔒 ℓ _↦_ = Σ (∀̇ π̂ ℓ 𝔒) (λ P → ∀ {x} {f g : 𝔒 x} → f ↦ g → Extension P f g)

ArrowExtendedṖroperty : ∀
  {𝔵} {𝔛 : Ø 𝔵}
  {𝔬₁} (𝔒₁ : 𝔛 → Ø 𝔬₁)
  {𝔬₂} (𝔒₂ : 𝔛 → Ø 𝔬₂)
  ℓ
...
ArrowExtendedṖroperty 𝔒₁ 𝔒₂ ℓ _↦_ x = ExtendedṖroperty (Arrow 𝔒₁ 𝔒₂ x) ℓ (Extended _↦_)
...
ArrowsourceExtendedṖroperty : ∀
  ...
  ℓ
  → (x : 𝔛)
  → ∀ {ℓ̇} (_↦_ : ∀̇ π̂² ℓ̇ (Arrow 𝔒₁ 𝔒₂ x))
  → Ø 𝔵 ∙̂ 𝔬₁ ∙̂ 𝔬₂ ∙̂ ↑̂ ℓ ∙̂ ℓ̇
ArrowsourceExtendedṖroperty {𝔬₁ = 𝔬₁} 𝔒₁ {𝔬₂} 𝔒₂ ℓ x _↦_ = ExtendedṖroperty (Arrow 𝔒₁ 𝔒₂ x) ℓ _↦_
