...
module _ where
  ...
  𝒻act3 : ∀ {𝔞} {𝔄 : Ø 𝔞} {𝔟} {𝔅 : 𝔄 → Ø 𝔟} {ℓ} → TYPE ℓ 𝔅 → Ø 𝔞 ∙̂ 𝔟 ∙̂ ℓ
  𝒻act3 {𝔅 = B} C = ∀ {a} {b : B a} → C b

  [𝓯act3] : ∀ {𝔞} {𝔄 : Ø 𝔞} {𝔯} {𝔟} {ℓ} (_∼ᵣ_ : π̂² 𝔯 𝔄) (B : π̂ 𝔟 𝔄) ⦃ _ : 𝓡eflexivity _∼ᵣ_ ⦄ ⦃ _ : 𝓢urjectextensivity _∼ᵣ_ B ⦄ ⦃ _ : ∀ {x} → HasEquivalence (B x) ℓ ⦄ → TYPE ℓ B
  [𝓯act3] _∼ᵣ_ 𝔅 B = B ≈ ε[ _∼ᵣ_ ] ◃ B

  module _
    {ℓ} {𝔞} {𝔄 : Ø 𝔞} {𝔟} {𝔅 : 𝔄 → Ø 𝔟}
    (type : TYPE ℓ 𝔅)
    where
    record [𝐹act3] 𝔯 : Ø 𝔞 ∙̂ 𝔟 ∙̂ ↑̂ 𝔯 ∙̂ ↑̂ ℓ where
      constructor ∁
      field
        _∼ᵣ_ : π̂² 𝔯 𝔄
        ...
        ⦃ ⌶CorrectFact3 ⦄ : (λ {a} → [𝓯act3] _∼ᵣ_ 𝔅 {a}) ≡ type

    record 𝐹act3 {𝔯} ⦃ _ : [𝐹act3] 𝔯 ⦄ : Ø 𝔞 ∙̂ 𝔟 ∙̂ ℓ where
      field fact3 : 𝒻act3 (λ {x} → type {x})
  ...
  module _
    ...
    where
    𝓯act3 = 𝒻act3 (λ {x} → [𝓯act3] _∼ᵣ_ 𝔅 {x})
    [𝓕act3] = [𝐹act3] (λ {x} → [𝓯act3] _∼ᵣ_ 𝔅 {x})
    𝓕act3 = 𝐹act3 (λ {x} → [𝓯act3] _∼ᵣ_ 𝔅 {x})
