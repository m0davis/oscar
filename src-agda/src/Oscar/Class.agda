...
module _ where
  ...
  𝓅roperfact1 : ∀ {𝔞} {𝔄 : Ø 𝔞} ℓ → (refl4 : 𝔄 → 𝔄 → 𝔄 → 𝔄 → Ø ℓ) → Ø 𝔞 ∙̂ ℓ
  𝓅roperfact1 ℓ refl4 = ∀ s1 s2 t1 t2 → refl4 s1 s2 t1 t2

  𝓹roperfact1 : ∀ {𝔞} {𝔄 : Ø 𝔞} {𝔟} {𝔅 : Ø 𝔟} {ℓ} ⦃ _ : Properthing ℓ 𝔅 ⦄ (_∼_ : 𝔄 → 𝔄 → 𝔅) (_⊛_ : 𝔄 → 𝔄 → 𝔄) → Ø 𝔞 ∙̂ ℓ
  𝓹roperfact1 _∼_ _⊛_ = let _∼_ = _∼_ ; infix 18 _∼_ in
    𝓅roperfact1 _ (λ s1 s2 t1 t2 → s1 ⊛ s2 ∼ t1 ⊛ t2 ≈ s1 ∼ t1 ∧ s2 ∼ t2)

  module _
    {𝔞} {𝔄 : Ø 𝔞} ℓ (refl4 : 𝔄 → 𝔄 → 𝔄 → 𝔄 → Ø ℓ)
    where
    record [𝒫roperfact1] 𝔟 : Ø 𝔞 ∙̂ ↑̂ 𝔟 ∙̂ ↑̂ ℓ where
      ...
      field
        ...
        _⊛_ : 𝔄 → 𝔄 → 𝔄
        ⦃ ⌶CorrectProp ⦄ : (λ s1 s2 t1 t2 → s1 ⊛ s2 ∼ t1 ⊛ t2 ≈ s1 ∼ t1 ∧ s2 ∼ t2) ≡ refl4

    record 𝒫roperfact1 {𝔟} ⦃ _ : [𝒫roperfact1] 𝔟 ⦄ : Ø 𝔞 ∙̂ ℓ where
      field properfact1 : 𝓅roperfact1 ℓ refl4
  ...
  module _
    ...
    where
    [𝓟roperfact1] = [𝒫roperfact1] _ (λ s1 s2 t1 t2 → s1 ⊛ s2 ∼ t1 ⊛ t2 ≈ s1 ∼ t1 ∧ s2 ∼ t2) 𝔟
    𝓟roperfact1 = 𝒫roperfact1 _ (λ s1 s2 t1 t2 → s1 ⊛ s2 ∼ t1 ⊛ t2 ≈ s1 ∼ t1 ∧ s2 ∼ t2)
