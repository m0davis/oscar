...
module Oscar.Property-NoInstanceBug1 where
...
record 𝓢urjectivity
  ...
  {𝔬₂} {𝔒₂ : Ø 𝔬₂}
  {𝔯₂} (_∼₂_ : 𝔒₂ → 𝔒₂ → Ø 𝔯₂)
  ⦃ _ : 𝓢urjection 𝔒₁ 𝔒₂ ⦄ ⦃ _ : [𝓢urjectivity] _∼₁_ _∼₂_ ⦄ : Ø 𝔬₁ ∙̂ 𝔯₁ ∙̂ 𝔬₂ ∙̂ 𝔯₂ where
  field
    surjectivity : ∀ {x y} → x ∼₁ y → surjection x ∼₂ surjection y
...
instance
  ...
  toSurj : ∀
    ...
    ⦃ _ : 𝓢urjectivity _∼₁_ (Extension 𝔒₂) ⦃ record { surjection = λ x → x } ⦄ ⦄
    → 𝓢urjectivity' _∼₁_ 𝔒₂
...
module _
  ...
  instance
    ...
    𝓢urjectionIdentity .𝓢urjection.surjection = λ x → x
...
module TestPropertyFunctions
  ...
  works2 : ∀ {x y} → ArrowsourceProperty 𝔒₁ 𝔒₂ ℓ x → Arrow 𝔒₁ 𝔒₂ x y → ArrowsourceProperty 𝔒₁ 𝔒₂ ℓ y
  works2 P f = surjectivity' f (λ {_} → P)
