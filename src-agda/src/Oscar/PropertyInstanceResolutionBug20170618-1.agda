...
module Oscar.PropertyInstanceResolutionBug20170618-1 where
...
module _ {𝔭} {𝔓 : Ø 𝔭} where
  ...
  mySub : Substitunction⌶ 𝔓
  mySub = ∁

  open Substitunction⌶ mySub -- (Substitunction⌶ 𝔓 ∋ ∁)
...
module _ where
  ...
  prop-id14 {m = m} {n} {f = f} P = π₁ P (symmetry {x = _} {y = λ 𝓞 → f 𝓞} (transleftidentity {_∼_ = _} {_∼̇_ = Proposextensequality}))
  ...
  prop-id15 {m = m} {n} {f = f} P = π₁ P (symmetry {x = λ 𝓞 →
                                                            𝓣ransitivity.transitivity
                                                              ! -- (Substitunction⌶.𝓣ransitivitySubstitunction mySub!)
                                                            {m} {n} {n}
                                                            (λ z → f z) (𝓡eflexivity.reflexivity 𝓡eflexivitySubstitunction {n})
                                                            𝓞} {y = λ 𝓞 → f 𝓞} (transleftidentity {_∼_ = _} {_∼̇_ = Proposextensequality} {x = m}))

  prop-id2 : ∀ {m n ℓ} {f : Substitunction m n} (P : 𝓔xtensionalProperty (Fin m) Term ℓ Proposextensequality[ Fin m / (λ _ → Term _) ]) (let P₀ = π₀ P) → P₀ f → P₀ (i ∙ f)
  prop-id2 {m = m} {n} {f = f} P = π₁ P (symmetry {x = λ 𝓞 →
                                                                                                                           𝓣ransitivity.transitivity 𝓣ransitivitySubstitunction {m} {n} {n}
                                                                                                                           (λ 𝓞₁ → f 𝓞₁)
                                                                                                                           (𝓡eflexivity.reflexivity 𝓡eflexivitySubstitunction {n}) 𝓞} {y = λ 𝓞 → f 𝓞} (transleftidentity {_∼_ = {!!}} {_∼̇_ = Proposextensequality}))

  prop-id21 : ∀ {m n ℓ} {f : Substitunction m n} (P : 𝓔xtensionalProperty (Fin m) Term ℓ Proposextensequality[ Fin m / (λ _ → Term _) ]) (let P₀ = π₀ P) → P₀ f → P₀ (i ∙ f)
  prop-id21 {m = m} {n} {f = f} P = π₁ P (symmetry {x = λ 𝓞 →
                                                                                                                           𝓣ransitivity.transitivity 𝓣ransitivitySubstitunction {m} {n} {n}
                                                                                                                           (λ 𝓞₁ → f 𝓞₁)
                                                                                                                           (𝓡eflexivity.reflexivity 𝓡eflexivitySubstitunction {n}) 𝓞} {y = λ 𝓞 → f 𝓞} (transleftidentity {_∼_ = λ z z₁ → (𝓞 : ¶⟨< z ⟩) → Term z₁} {_∼̇_ = Proposextensequality}))

  prop-id3 : ∀ {m n ℓ} {f : Substitunction m n} (P : 𝓔xtensionalProperty (Fin m) Term ℓ Proposextensequality[ Fin m / (λ _ → Term _) ]) (let P₀ = π₀ P) → P₀ f → P₀ (i ∙ f)
  prop-id3 {m = m} {n} {f = f} P = π₁ P (≡̇-symmetry {𝔒 = ¶⟨< m ⟩} {𝔓 = λ _ → Term n} {x = λ 𝓞 →
                                                                                                                           𝓣ransitivity.transitivity 𝓣ransitivitySubstitunction {m} {n} {n}
                                                                                                                           (λ 𝓞₁ → f 𝓞₁)
                                                                                                                           (𝓡eflexivity.reflexivity 𝓡eflexivitySubstitunction {n}) 𝓞} {y = λ 𝓞 → f 𝓞} (transleftidentity {_∼_ = λ z z₁ → (𝓞 : ¶⟨< z ⟩) → Term z₁} {_∼̇_ = Proposextensequality}))
