...
module _
  ...
  {a₂} {A₂ : Ø a₂} {b₂}
    ...
    ⦃ _ : Map B₁ (B₂ on μ) ⦄
  where
  𝓬ommutativity : Ø a₁ ∙̂ b₁ ∙̂ c₂
  𝓬ommutativity = ∀ {x y} (f : B₁ x y) {z} (g : B₁ y z) → map[ B₂ on μ ] (g ∙ f) ≋ map g ∙ map f
  record Commutativity : Ø a₁ ∙̂ b₁ ∙̂ a₂ ∙̂ b₂ ∙̂ ↑̂ c₂ where
    field
      commutativity : ∀ {x y} (f : B₁ x y) {z} (g : B₁ y z) → map[ B₂ on μ ] (g ∙ f) ≋ map g ∙ map f
...
test-commutativity : ∀
  ...
  → ∀ {x y} (f : B₁ x y) {z} (g : B₁ y z) → map[ B₂ on μ ] (g ∙ f) ≋ map g ∙ map f
test-commutativity B₁ B₂ c₂ μ f g = commutativity f g
