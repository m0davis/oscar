...
terms-inj : ∀ {𝑎} → {ts1 ts2 : Vec Term (arity 𝑎)} → (τ₁≡τ₂ : (Terms.⟨_⟩ {𝑎} ts1) ≡ (⟨_⟩ ts2)) → ts1 ≡ ts2
terms-inj refl = refl
...
mutual
  ...
  … | yes refl = EqVecTerm⇑ τ₁s τ₂s >>= λ { (yes refl) → now $ yes refl ; (no τ₁s≢τ₂s) → now $ no (λ ⟨τ₁s⟩≡⟨τ₂s⟩ → τ₁s≢τ₂s (terms-inj ⟨τ₁s⟩≡⟨τ₂s⟩)) }
