...
module UnifyMguCorrectL (A : Set) ⦃ isDecEquivalenceA : IsDecEquivalence (_≡_ {A = A}) ⦄ where

open import UnifyFin
open import UnifyTermL A
open import UnifyMguL A
...
check-prop x (leaf l) = inj₁ (leaf l , (refl , refl))
...
data Amgu : {m : ℕ} -> (s t : Term m) -> ∃ (AList m) -> Maybe (∃ (AList m)) -> Set where
  ...
  leaf-leaf : ∀ {m l acc} ->           Amgu {m}     (leaf l)     (leaf l)     acc        (just acc)
  leaf-leef : ∀ {m l₁ l₂ acc} -> l₁ ≢ l₂ →
                                       Amgu {m}     (leaf l₁)    (leaf l₂)     acc       nothing
  leaf-fork : ∀ {m l s t acc} ->       Amgu {m}     (leaf l)     (s fork t)   acc        nothing
...
open IsDecEquivalence isDecEquivalenceA using () renaming (_≟_ to _≟A_)

view : ∀ {m : ℕ} -> (s t : Term m) -> (acc : ∃ (AList m)) -> Amgu s t acc (amgu s t acc)
view (leaf l₁)    (leaf l₂)    acc
 with l₁ ≟A l₂
… | yes refl = leaf-leaf
… | no l₁≢l₂ = leaf-leef l₁≢l₂
view (leaf l)     (s fork t)   acc        = leaf-fork
view (s fork t)   (leaf l)     acc        = Flip refl leaf-fork
view (s1 fork s2) (t1 fork t2) acc        = fork-fork
view (i x)        (i y)        (m , anil) = var-var
view (i x)        (leaf l)     (m , anil) = var-t (λ ())
view (i x)        (s fork t)   (m , anil) = var-t (λ ())
view (leaf l)     (i x)        (m , anil) = Flip refl (var-t (λ ()))
view (s fork t)   (i x)        (m , anil) = Flip refl (var-t (λ ()))
view (i x)        (i x')       (n , σ asnoc r / z) = s-t
view (i x)        (leaf l)     (n , σ asnoc r / z) = s-t
view (i x)        (s fork t)   (n , σ asnoc r / z) = s-t
view (leaf l)     (i x)        (n , σ asnoc r / z) = s-t
...
No[P◇ρ]-fork : ∀ {m l} (s1 t1 : Term m) (ρ : AList m l) (s2 t2 : Term m) →
               Nothing⋆ $ Unifies⋆ s1 t1 [-◇⋆ sub ρ ] →
               Nothing⋆ $ Unifies⋆ (s1 fork s2) (t1 fork t2) [-◇⋆ sub ρ ]
No[P◇ρ]-fork s1 t1 ρ s2 t2 nounify = No[Q◇ρ]→No[P◇ρ] No[Q◇ρ]
  where
...
No[P◇ρ]-fork' : ∀ {m l n} (σ : AList l n)
                  ...
                  Nothing⋆ $ Unifies⋆ (s1 fork s2) (t1 fork t2) [-◇⋆ sub ρ ]
No[P◇ρ]-fork' σ s1 s2 t1 t2 ρ a nounify = No[Q◇ρ]→No[P◇ρ]⋆ No[Q◇ρ]⋆
    where
    P⋆ = Unifies⋆ (s1 fork s2) (t1 fork t2)
    Q⋆ = (Unifies⋆ s1 t1 ∧⋆ Unifies⋆ s2 t2)
    Q⇔P⋆ : Q⋆ ⇔⋆ P⋆
...
No-Unifier' : ∀ {l} (x : Fin (suc l)) (t : Term (suc l)) →
              i x ≢ t →
              (ps : List (Step (suc l))) →
              t ≡ (ps ⊹ i x) →
              ∀ {n} (f : Fin (suc l) → Term n) → f x ≢ (f ◃ t)
No-Unifier' {l} x t ix≢t ps r = No-Unifier
  where
...
amgu-c {l = l} {ρ} (leaf-leef l₁≢l₂) | ._
  = inj₂ ((λ f x → l₁≢l₂ (Term-leaf-inj x)) , refl)
amgu-c {l = l} {ρ} (leaf-leaf {l = L}) | ._
  = inj₁ (l , anil , trivial-problem {_} {_} {leaf L} {sub ρ} , cong (λ x -> just (l , x)) (sym (SubList.anil-id-l ρ)) )
...
… | .nothing             | inj₂ (nounify , refl) = inj₂ ((λ {_} -> No[P◇ρ]-fork s1 t1 ρ s2 t2 nounify) , refl)
… | .(just (n , σ ++ ρ)) | inj₁ (n , σ , a , refl)
 with amgu s2 t2 (n , σ ++ ρ) | amgu-c (view s2 t2 (n , (σ ++ ρ)))
… | .nothing                | inj₂ (nounify , refl) = inj₂ ( (λ {_} -> No[P◇ρ]-fork' σ s1 s2 t1 t2 ρ a nounify) , refl)
… | .(just (n1 , σ1 ++ (σ ++ ρ))) | inj₁ (n1 , σ1 , b , refl)
    = inj₁ (n1 , σ1 ++ σ , Max[P∧Q◇ρ][σ1++σ]' s1 s2 t1 t2 ρ σ a σ1 b , cong (λ σ -> just (n1 , σ)) (++-assoc σ1 σ ρ))
...
… | .nothing  | inj₂ ( ps , r , refl) = inj₂ ( (λ {_} -> No-Unifier' x t ix≢t ps r )  , refl)
...
… | inj₂ (nounify , ra) = inj₂ ( (λ {_} -> NoQ→NoP' s t ρ r z nounify) , cong (_<$>_ (λ x' → x' ∃asnoc r / z)) ra )
… | inj₁ (n , σ , a , ra)  = inj₁ (n , σ , MaxQ→MaxP s t ρ r z σ a , cong (_<$>_ (λ x' → x' ∃asnoc r / z)) ra)
