...
;(when (null (find-package "OSCAR")) (make-package "OSCAR"))
;
;(in-package "OSCAR")
...
(defun udos (n) (undefeated-degree-of-support n))

(defun mdos (n) (maximal-degree-of-support n))
...
(defun compute-maximal-degree-of-support (node)
  (cond
    (*deductive-only* 1.0)
    ((temporal-node node)
...
(defun compute-undefeated-degree-of-support (node)
  (cond
    (*deductive-only* 1.0)
    ((temporal-node node)
...
(defun compute-old-undefeated-degree-of-support (node)
...
(defun display-inference-node (n )
...
(defun display-unsupported-node (n )
  (if (numberp n) (setf n (node n)))
...
(when (null *trees-loaded*)
  (load (concatenate 'string oscar-pathname "Assignment-trees_3-26"))
  (setf *trees-loaded* t))
...
(defun inclusive-node-ancestors (node)
...
(defun store-inference-node (node formula)
...
#| (descrimination-tests d-node) is an a-list of pairs (test . dn), where test has the form of the
car of a formula-code, and dn is a d-node. |#
(defun index-inference-node (node profile term-list d-node)
  (let ((dn (e-assoc (car profile) (discrimination-tests d-node)))
        (new-profile (cdr profile)))
    (cond (dn
...
(defun store-inference-node-at-d-node (node term-list dn)
...
#| Test is the final member of the formula-profile for the node-formula. |#
(defun store-inference-node-at-new-d-node (node term-list d-node test)
...
(defun index-inference-node-at-new-nodes (node term-list d-node profile test)
  (let ((dn (make-d-node
              :d-node-number (incf *d-node-number*)
              :d-node-description (cons test (d-node-description d-node))
              :parent-d-node d-node)))
...
(defun store-inference-node-with-c-list (node formula c-list)
...
(defun store-processed-inference-node (node)
...
(defun premise-node-specifier (premise)
...
in the Listener. |#
(defun graphics-pause-on ()
  (setf *graphics-pause* t))
(defun graphics-pause-off ()
...
(defun reason-degenerately-backwards-from-dominant-reason-nodes (interest priority depth d-node)
...
(defun invert-contradictions-from-new-support-link (link instantiations)
...
(defun reductio-supposition (node) (eq (node-justification node) :reductio-supposition))
...
(defun build-support-link (basis clues rule discount node NDA binding link instantiations depth defeasible?)
...
(defun recursively-compute-node-ancestors (node ancestors)
  (dolist (L (consequent-links node))
...
(defun add-support-link (link node depth)
...
(defun compute-node-dependencies (node indent msg)
  (when *d-trace*
...
(defun anchor-node-relative-to-interest (node I indent msg)
...
(defun anchor-node-relative-to-node (node N0 indent msg)
  (pull node *dependent-nodes*)
  (when *d-trace*
    (bar-indent indent) (princ msg) (princ node) (terpri))
  (dolist (n (node-consequences node))
...
(defun node-descendants-of-interest (interest)
...
(defun recompute-descendant-udoss (node)
  (dolist (L (consequent-links node))
...
;(defun tree-degree (node assignment-tree)
...
(defun formula-node-variables (p)
...
(when (equal (lisp-implementation-type) "Macintosh Common Lisp")
  (load (concatenate 'string oscar-pathname "mac-stuff")))
...
(when (null *problems-loaded*)
  ...
  (load (concatenate 'string oscar-pathname "Combined-problems"))
