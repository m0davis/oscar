...
(defun assignment-tree (n)
  (catch 'number
         (find-tree n *assignment-tree*)))

(defun find-tree (n assignment-tree)
...
(defun triangle-set (n)
  (catch 'number
         (find-triangle n *assignment-tree*)))

(defun find-triangle (n assignment-tree)
...
(defun compute-triangle-domains (tree)
  ; (setf *tree* tree)
  (let ((Y nil))
...
(defun tree-node-MPUD* (node tree)
...
(defun tree-link-MPUD* (link tree)
...
(defun tree-node-UD (node assignment-tree)
  (if assignment-tree
...
#| If node has an assigned status in assignment-tree or one of its ancestors,
this returns (node.status). |#
(defun tree-node-MPUD (node assignment-tree)
  (if assignment-tree
...
(defun tree-link-UD (link assignment-tree)
  (if assignment-tree
...
(defun make-node-assoc (node value)
  (cond ((temporal-node node) (cons node (cons value *cycle*)))
        (t (cons node value))))

(defun make-link-assoc (link value)
...
(defun inference/defeat-ancestors (X tree &optional ancestors)
...
(defun compute-assignment-tree (&optional assignment parent-tree (indent-level 0))
...
(defun closure-with-consistency-check
...
(defun dual-closure-with-consistency-check
  (arb-link-UD arb-link-MPUD arb-node-UD arb-node-MPUD tree
...
(defun dictated-values (arb-UD arb-MPUD tree D1 D2 link-UD link-MPUD node-UD node-MPUD)
...
(defun dictated-values-from-support-links
...
(defun dictated-values-from-nodes
...
(defun node-assoc-UD (node UDs UD tree)
...
(defun node-assoc-MPUD (node MPUDs MPUD tree)
...
(defun link-assoc-UD (node UDs UD tree)
  (let ((assoc  (assoc node UDs)))
    (when (null assoc)
      (setf assoc (assoc node UD))
      (when (and (null assoc) tree)
...
(defun link-assoc-MPUD (node MPUDs MPUD tree)
  (let ((assoc  (assoc node MPUDs)))
    (when (null assoc)
      (setf assoc (assoc node MPUD))
      (when (and (null assoc) tree)
...
(defun compute-triangle-set (triangle tree &optional (indent-level 0))
  (let ((triangle-set
          (make-triangle-set
...
(defun compute-assignments-to-domain (tree triangle)
...
(defun compute-support-link-strength (link)
...
(defun support-link-maximal-strength (link)
  (let ((reason-strength
...
(defun correct-for-definite-defeat-status ()
...
(defun inference/defeat-descendants
...
(defun inference/defeat-node-descendants
...
(defun new-computable-node-values (node ass-link-UD ass-link-MPUD)
...
(defun new-computable-link-values (link ass-UD ass-MPUD)
...
(defun compute-status-assignments (effects &optional assignment-tree)
...
(defun update-assignment-tree (links tree effects &optional parent-tree)
...
(defun recompute-triangle-sets (links tree effects)
...
(defun update-triangle-set (links triangle-set effects tree)
