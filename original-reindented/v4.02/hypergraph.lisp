...
(defunction ultimate-target (link)
...
(defunction defeat-paths (link1 link2 &optional used-links)
     (let ((d-paths nil)
...
(defunction clear-hyperlink-defeat-loops ()
     (dolist (link *hyperlinks*) (setf (hyperlink-defeat-loops link) T)))

(defunction clear-criticalities ()
...
(defunction critical-link (DL sigma)
...
(defunction sigma-defeat-loop (d-loop sigma)
    (or (null (cdr sigma)) (every #'(lambda (L) (mem (cdr sigma) (hyper-defeat-link-in L))) d-loop)))

;; DL is a hyper-defeat-link and sigma a list of hyperlinks and unit sets of hyperlinks.
(defunction hereditarily-critical-link (DL sigma)
...
(defunction critical-hyperlinks (DL sigma)
     (let ((L (car sigma))
               (critical-links nil))
        (when (hyperlink-p L)
...
(defunction convergent-support-path (path sigma DL)
     (let ((L (car sigma)))
        (and (hyperlink-p L)
                   (some     ;; (defeat-loops L)
                     #'(lambda (d-loop)
...
(defunction subsidiary-hyperlinks-for-node (node sigma path DL indent)
...
(defunction subsidiary-hyperlinks-for-link (link sigma path DL indent)
     (when (and *display?* *safe-trace*)
...
(defunction clear-graph-memories ()
...
(defunction compute-new-hypergraphs (link &optional sigma (indent 0))
...
(defunction display-split (link sigma indent)
     (terpri)
...
(defunction add-node-to-critical-graph (node sigma1 sigma2 &optional (indent 0))
...
(defunction add-hyper-defeat-link-to-critical-graph (dl sigma1 sigma2 &optional (indent 0))
...
(defunction display-hypergraphs (link)
...
(defunction compute-hypernode-degrees-of-justification ()
...
(defunction reset-memories (link)
...
(defunction compute-affected-justifications (link)
...
;; can be reused in computing the degree of justification of the first member of sigma. This returns
;; the previously computed-value.
(defunction independent-hypernode-value (node sigma)
...
(defunction compute-hypernode-justification (node sigma &optional link0 (indent 0) case path)
...
(defunction last-link (values path)
     (let ((last-link nil)
               (remainder path))
        (dolist (value values)
...
(defunction compute-link-justification (link sigma link0 &optional (indent 0) path)
...
(defunction compute-hyper-defeat-link-justification (DL sigma link0 &optional (indent 0) path)
...
(defunction critical-set (links paths)
